...
class Moe(nn.Module):
    ...
    def forward(self, z_moe):
        z_moe_pool = z_moe.mean(dim=0) # (seq_len, input_dim)
        gate_logits = self.gate_net(z_moe_pool) # (seq_len, num_domains)
        gate_weights = F.softmax(gate_logits, dim=-1)
        return gate_weights  # (seq_len, num_domains)
...
class MoeGSAN(nn.Module):
    ...
    def forward(self, seqs, neg_seqs=None, aug_seqs=None):
        ...
        for i in range(self.num_domains):
            result_moe += result_weight[:, i].unsqueeze(-1).unsqueeze(0) * result_list[i].detach()
